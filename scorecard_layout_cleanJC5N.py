...
# Group by round
rounds = df["Round Link"].unique()
selected_round = st.selectbox("Select a Round", rounds)
round_data = df[df["Round Link"] == selected_round]

# Metadata
player = round_data["Player Name"].iloc[0]
course = round_data["Course Name"].iloc[0]
date = pd.to_datetime(round_data["Date Played"].iloc[0]).strftime("%B %d, %Y")
...
with right_col:
    ...
    <div style="margin-top:10px;">
      ...
      <div style="background:#222; padding:10px; border-radius:10px; color:#fff; font-size:13px; line-height:1.6;">
        <b>📊 Round Totals</b><br>
