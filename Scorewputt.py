...
# Tabs
# =========================================================
tab_scorecard, tab_overview, tab_analysis, tab_putting, tab_putting_stats, tab_trends, tab_bestof, tab_shortgame = st.tabs(["Scorecard", "Overview", "Analysis", "Putting", "Putting Stats", "Trends", "Best Of", "Short Game"])
...
with tab_putting_stats:
    ...
    def _puttstats_n(series, default=0):
        return pd.to_numeric(series, errors="coerce").fillna(default)

    def _puttstats_as_int(series, default=0):
        return _puttstats_n(series, default=default).astype(int)

    def _puttstats_pct(numer, denom):
        return (numer / denom * 100.0) if denom else 0.0

    def _puttstats_pct_str(x):
        try:
            return f"{float(x):.1f}%"
        except Exception:
            return "—"

    def _puttstats_norm_yes_no(v):
        if pd.isna(v):
            return pd.NA
        s = str(v).strip().lower()
    ...
    puttstats_df = df.copy()
    puttstats_df["Date Played"] = pd.to_datetime(puttstats_df.get("Date Played"), errors="coerce")
    puttstats_df["Year"] = puttstats_df["Date Played"].dt.year
    ...
    for col in [
        ...
        if col not in puttstats_df.columns:
            puttstats_df[col] = pd.NA

    puttstats_df["GIR"] = puttstats_df["GIR"].apply(_puttstats_norm_yes_no)
    puttstats_df["Approach GIR"] = puttstats_df["Approach GIR"].apply(_puttstats_norm_yes_no)

    puttstats_df["Putts"] = _puttstats_as_int(puttstats_df["Putts"], 0)
    puttstats_df["Hole"] = _puttstats_as_int(puttstats_df["Hole"], 0)

    par_num = pd.to_numeric(puttstats_df.get("Par"), errors="coerce").round()
    puttstats_df["Par"] = par_num.where(par_num.isin([3, 4, 5]), pd.NA).astype("Int64")

    puttstats_df["HoleScoreN"] = pd.to_numeric(puttstats_df[SCORE_COL], errors="coerce")
    puttstats_df["ProxN"] = pd.to_numeric(puttstats_df[PROX_COL], errors="coerce")
    puttstats_df["YardN"] = pd.to_numeric(puttstats_df[YARD_COL], errors="coerce")
    puttstats_df["FeetMadeN"] = pd.to_numeric(puttstats_df[FEET_MADE_COL], errors="coerce")
    puttstats_df["3 Putt Bogey"] = _puttstats_as_int(puttstats_df["3 Putt Bogey"], 0)

    years_available = sorted([int(y) for y in puttstats_df["Year"].dropna().unique().tolist()])
    year_options = ["All"] + years_available
    sel_year = st.selectbox("Year", year_options, index=0, key="puttstats_year")

    if sel_year != "All":
        puttstats_df = puttstats_df[puttstats_df["Year"] == int(sel_year)].copy()

    if puttstats_df.empty:
        ...
        with c1:
            players = sorted([x for x in puttstats_df["Player Name"].dropna().unique().tolist() if str(x).strip() != ""])
            sel_player = st.selectbox("Player", ["All"] + players, key="puttstats_player")
        with c2:
            courses = sorted([x for x in puttstats_df["Course Name"].dropna().unique().tolist() if str(x).strip() != ""])
            sel_course = st.selectbox("Course", ["All"] + courses, key="puttstats_course")
        with c3:
            pars = [3, 4, 5]
            sel_pars = st.multiselect("Par", pars, default=pars, key="puttstats_pars")
        with c4:
            clubs = sorted([x for x in puttstats_df[CLUB_COL].dropna().unique().tolist() if str(x).strip() != ""])
            sel_clubs = st.multiselect("Approach Club", clubs, default=[], key="puttstats_clubs")
            sel_gir = st.selectbox("GIR", ["All", "Yes", "No"], key="puttstats_gir")
            sel_appr_gir = st.selectbox("Approach GIR", ["All", "Yes", "No"], key="puttstats_appr_gir")

        c5, c6 = st.columns([2.2, 1.8], vertical_alignment="bottom")
        with c5:
            holes = sorted([int(x) for x in puttstats_df["Hole"].dropna().unique().tolist() if int(x) > 0])
            sel_holes = st.multiselect("Hole Number", holes, default=[], key="puttstats_holes")
        with c6:
            yard_series = puttstats_df["YardN"].dropna()
        ...
        if sel_clubs:
            f = f[f[f"{CLUB_COL}"].isin(sel_clubs)]
        if sel_holes:
        ...
        if f.empty:
            ...
            def build_puttstats_table(frame: pd.DataFrame) -> pd.DataFrame:
                tmp = frame.copy()
                tmp["PuttsN"] = _puttstats_as_int(tmp["Putts"], 0)
                ...
                for label, mask in buckets:
                    ...
                    bog3 = int(_puttstats_as_int(b["3 Putt Bogey"], 0).sum())

                    feet_made_total = float(_puttstats_n(b["FeetMadeN"], 0).sum())
                    ...
                    rows.append({
                        ...
                        "1-putts": p1,
                        "1-putt %": round(_puttstats_pct(p1, holes), 1),
                        "2-putts": p2,
                        "2-putt %": round(_puttstats_pct(p2, holes), 1),
                        "3+ putts": p3p,
                        "3+ putt %": round(_puttstats_pct(p3p, holes), 1),
                        "3-putt bogeys": bog3,
                        "3-putt bogey %": round(_puttstats_pct(bog3, holes), 1),
                        "Birdie+ Attempts": birdie_attempts,
                        "Birdie+ Makes": birdie_makes,
                        "Birdie+ %": round(_puttstats_pct(birdie_makes, birdie_attempts), 1),
                        "Par Attempts": par_attempts,
                        "Par Makes": par_makes,
                        "Par %": round(_puttstats_pct(par_makes, par_attempts), 1),
                ...
                if not out.empty:
                    ...
                    total_row["1-putt %"] = round(_puttstats_pct(int(total_row["1-putts"]), total_holes), 1)
                    total_row["2-putt %"] = round(_puttstats_pct(int(total_row["2-putts"]), total_holes), 1)
                    total_row["3+ putt %"] = round(_puttstats_pct(int(total_row["3+ putts"]), total_holes), 1)
                    total_row["3-putt bogey %"] = round(_puttstats_pct(int(total_row["3-putt bogeys"]), total_holes), 1)

                    total_ba = int(total_row["Birdie+ Attempts"])
                    total_bm = int(total_row["Birdie+ Makes"])
                    total_row["Birdie+ %"] = round(_puttstats_pct(total_bm, total_ba), 1)

                    total_pa = int(total_row["Par Attempts"])
                    total_pm = int(total_row["Par Makes"])
                    total_row["Par %"] = round(_puttstats_pct(total_pm, total_pa), 1)
            ...
            table = build_puttstats_table(f)
            percent_cols = ["1-putt %", "2-putt %", "3+ putt %", "3-putt bogey %", "Birdie+ %", "Par %"]

            table_display = table.copy()
            for col in percent_cols:
                table_display[col] = table_display[col].apply(_puttstats_pct_str)

            def _puttstats_style_table(df_show: pd.DataFrame):
            ...
            st.dataframe(_puttstats_style_table(table_display), use_container_width=True, hide_index=True)
            ...
            chart_plot["Label"] = chart_plot.apply(_puttstats_label_row, axis=1)
            ...
            long_df["Label"] = long_df.apply(_puttstats_mk_label, axis=1)
